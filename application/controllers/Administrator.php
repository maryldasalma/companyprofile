...
class Administrator extends CI_Controller
{
            ...
            if ($this->input->post()) {
                ...
                if ($total > 0) {
                    ...
                    if (password_verify($password, $hash)) {
                        ...
                        $data = array('user_name'=>$this->db->escape_str($this->input->post('a')),
                            'password'=>$this->db->escape_str($this->input->post('b')),
                            'tanggal'=>date('Y-m-d H:i:s'),
                            'hitung'=>'1');
						$this->model_app->insert('user_error',$data);
						//cek jumlah error
						$jumlahsalah=$this->db->query("SELECT * FROM user_error WHERE user_name='".$this->db->escape_str($this->input->post('a'))."' AND hitung='1'")->num_rows();
						if($jumlahsalah>=9)
						{
							$data = array('blokir'=>'Y');
							$where = array('username' => $this->db->escape_str($this->input->post('a')));
							$this->model_app->update('users', $data, $where);

							$data = array('hitung'=>'0');
							$where = array('user_name' => $this->db->escape_str($this->input->post('a')));
							$this->model_app->update('user_error', $data, $where);
						}
                    ...
                    echo $this->session->set_flashdata('message', '<div class="alert alert-danger"><center>username salah atau akun anda sedang diblokir</center></div>');
    ...
    function tambah_playlist()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG|MP4';
    ...
    public function about() {
        ...
        if (isset($_POST['submit'])) {
            $data = array(
                'tagline' => $this->input->post('a'),
                'title' => $this->input->post('b'),
                'text' => $this->input->post('c'),
                'motto' => $this->input->post('d'),
                'vision' => $this->input->post('e'),
                'mission' => $this->input->post('f'),
            );
            ...
            $proses = $this->model_app->edit('about', array('id' => 1))->row_array(); 
            $data = array('record' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_about/view_about', $data);
        }
    } 

    // Controller Modul History

    public function history() {
        cek_session_akses('history', $this->session->id_session);

        if (isset($_POST['submit'])) {
            $data = array(
                'history' => $this->input->post('a'),
                'logo' => $this->input->post('b'),
                'pemilik' => $this->input->post('c'),
                'jenis' => $this->input->post('d'),
                'tanggal' => $this->input->post('e'),
                'twitter' => $this->input->post('f'),
                'facebook' => $this->input->post('g'),
                'instagram' => $this->input->post('h'),
            );
    
            $where = array('id' => $this->input->post('id'));
            $this->model_app->update('history', $data, $where);
    
            redirect('administrator/history'); 
        } else {
            $proses = $this->model_app->edit('history', array('id' => 1))->row_array(); 
            $data = array('record' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_history/view_history', $data);
        }
    }  
    
    // Controller Modul Contact

    public function contact() {
        cek_session_akses('contact', $this->session->id_session);

        if (isset($_POST['submit'])) {
            $data = array(
                'about' => $this->input->post('a'),
                'address' => $this->input->post('b'),
                'contact' => $this->input->post('c'),
                'gmaps' => $this->input->post('d'),
            );
    
            $where = array('id' => $this->input->post('id'));
            $this->model_app->update('contact', $data, $where);
    
            redirect('administrator/contact'); 
        } else {
            $proses = $this->model_app->edit('contact', array('id' => 1))->row_array(); 
            $data = array('record' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_contact/view_contact', $data);
        }
    }    

    // Controller Modul Gambar

    function gambar()
    {
        cek_session_akses('gambar', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_join_one('gambar', 'playlistgambar', 'id_playlistgambar', 'id_gambar', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_join_where('gambar', 'playlistgambar', 'id_playlistgambar', array('gambar.username' => $this->session->username), 'id_gambar', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_gambar/view_gambar', $data);
    }
    ...
    function tambah_gambar()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    'id_playlistgambar' => $this->input->post('a'),
                    'username' => $this->session->username,
                    'jdl_gambar' => $this->input->post('b'),
                    'gambar_seo' => seo_title($this->input->post('b')),
                    'keterangan' => $this->input->post('c'),
                    'gambar' => '',
                    'dilihat' => '0',
                    'hari' => hari_ini(date('w')),
            ...
            $this->model_app->insert('gambar', $data);
            redirect('administrator/gambar');
        } else {
            $data['record'] = $this->model_app->view_ordering('playlistgambar', 'id_playlistgambar', 'DESC');
            $data['tag'] = $this->model_app->view_ordering('taggambar', 'id_taggambar', 'DESC');
            $this->template->load('administrator/template', 'administrator/mod_gambar/view_gambar_tambah', $data);
        }
    }
    ...
    function edit_gambar()
    {
        ...
        if (isset($_POST['submit'])) {
            ...
            $this->model_app->update('gambar', $data, $where);
            redirect('administrator/gambar');
        } else {
            $record = $this->model_app->view_ordering('playlistgambar', 'id_playlistgambar', 'DESC');
            $tag = $this->model_app->view_ordering('taggambar', 'id_taggambar', 'DESC');
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('gambar', array('id_gambar' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('gambar', array('id_gambar' => $id, 'username' => $this->session->username))->row_array();
            }

            $data = array('rows' => $proses, 'record' => $record, 'tag' => $tag);
            $this->template->load('administrator/template', 'administrator/mod_gambar/view_gambar_edit', $data);
        }
    }
    ...
    function delete_gambar()
    {
        cek_session_akses('gambar', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_gambar' => $this->uri->segment(3));
        } else {
            $id = array('id_gambar' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('gambar', $id);
        redirect('administrator/gambar');
    }

    // Controller Modul Daftargambar

    function daftargambar(){
        cek_session_akses('daftargambar',$this->session->id_session);
        $data['record'] = $this->model_app->view_ordering('daftargambar','id_daftargambar','DESC');
		
	   $this->template->load('administrator/template','administrator/mod_daftargambar/view_daftargambar',$data);
    }

    function tambah_daftargambar() {
        cek_session_akses('daftargambar', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/img_daftargambar/';
            $config['allowed_types'] = 'gif|jpg|png|JPG|JPEG';
            $config['max_size'] = '500000'; // kb
            $this->load->library('upload', $config);
            $this->upload->do_upload('b');
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    'username' => $this->session->username,
                    'daftargambar_seo' => seo_title($this->input->post('a')),
                    'kategori' => seo_title($this->input->post('kategori')),
                    'jdl_daftargambar' => $this->input->post('a')
                    //'aktif' => 'Y'
                );
            } else {
                $data = array(
                    ...
                    'daftargambar_seo' => seo_title($this->input->post('a')),
                    'kategori' => seo_title($this->input->post('kategori')),
                    'gbr_daftargambar' => $hasil['file_name'],
                    'jdl_daftargambar' => $this->input->post('a')
                    //'aktif' => 'Y'
                );
            }
            $this->model_app->insert('daftargambar', $data);  
            redirect('administrator/daftargambar');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_daftargambar/view_daftargambar_tambah');
        }
    }    

    function edit_daftargambar() {
        cek_session_akses('daftargambar', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/img_daftargambar/';
            ...
            $where = array('id_daftargambar' => $this->input->post('id'));
            $this->model_app->update('daftargambar', $data, $where);
            redirect('administrator/daftargambar');
        } else {
            $proses = $this->model_app->edit('daftargambar', array('id_daftargambar' => $id))->row_array();
            $data = array('rows' => $proses); // Hanya mengirim data 'rows'
            $this->template->load('administrator/template', 'administrator/mod_daftargambar/view_daftargambar_edit', $data);
        }
    }    

    function delete_daftargambar(){
        cek_session_akses('daftargambar',$this->session->id_session);
        $id = $this->uri->segment(3);
        $this->model_app->delete('daftargambar', array('id_daftargambar' => $id));
        redirect('administrator/daftargambar');
    }

    // Controller Modul Playlist

    function playlistgambar()
    {
        cek_session_akses('sampulgambar', $this->session->id_session);
        $data['detailgambar'] = $this->model_app->view_ordering('sampulgambar', 'id_sampulgambar', 'DESC');
        $this->template->load('administrator/template', 'administrator/mod_playlistgambar/view_playlistgambar', $data);
    }

    function tambah_playlistgambar()
    {
        cek_session_akses('sampulgambar', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $config['upload_path'] = 'asset/img_playlist/';
            ...
            if ($hasil['file_name'] == '') {
                $data = array(
                    'jdl_sampulgambar' => $this->input->post('a'),
                    'username' => $this->session->username,
                    'sampulgambar_seo' => seo_title($this->input->post('a')),
                    'page' => $this->input->post('c')
                );
            } else {
                $data = array(
                    'jdl_sampulgambar' => $this->input->post('a'),
                    'username' => $this->session->username,
                    'sampulgambar_seo' => seo_title($this->input->post('a')),
                    'gbr_sampulgambar' => $hasil['file_name'],
                    'page' => $this->input->post('c')
                );
            }
            $this->model_app->insert('sampulgambar', $data);
            redirect('administrator/playlistgambar');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_playlistgambar/view_playlistgambar_tambah');
        }
    }   

    function edit_playlistgambar()
    {
        cek_session_akses('sampulgambar', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_sampulgambar' => $this->input->post('id'));
            $this->model_app->update('sampulgambar', $data, $where);
            redirect('administrator/playlistgambar');
        } else {
            $proses = $this->model_app->edit('sampulgambar', array('id_sampulgambar' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_playlistgambar/view_playlistgambar_edit', $data);
        }
    }    

    function delete_playlistgambar()
    {
        cek_session_akses('sampulgambar', $this->session->id_session);
        $id = array('id_sampulgambar' => $this->uri->segment(3));
        $this->model_app->delete('sampulgambar', $id);
        redirect('administrator/playlistgambar');
    }

    // Controller Modul Tag Gambar

    function taggambar()
    {
        cek_session_akses('taggambar', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $data['record'] = $this->model_app->view_ordering('taggambar', 'id_taggambar', 'DESC');
        } else {
            $data['record'] = $this->model_app->view_where_ordering('taggambar', array('username' => $this->session->username), 'id_taggambar', 'DESC');
        }
        $this->template->load('administrator/template', 'administrator/mod_taggambar/view_tag', $data);
    }

    function tambah_taggambar()
    {
        cek_session_akses('taggambar', $this->session->id_session);
        if (isset($_POST['submit'])) {
            $data = array(
                'nama_taggambar' => $this->db->escape_str($this->input->post('a')),
                'username' => $this->session->username,
                'taggambar_seo' => seo_title($this->input->post('a')),
                'count' => '0'
            );
            $this->model_app->insert('taggambar', $data);
            redirect('administrator/taggambar');
        } else {
            $this->template->load('administrator/template', 'administrator/mod_taggambar/view_tag_tambah');
        }
    }

    function edit_taggambar()
    {
        cek_session_akses('taggambar', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            ...
            $where = array('id_taggambar' => $this->input->post('id'));
            $this->model_app->update('taggambar', $data, $where);
            redirect('administrator/taggambar');
        } else {
            if ($this->session->level == 'admin') {
                $proses = $this->model_app->edit('taggambar', array('id_taggambar' => $id))->row_array();
            } else {
                $proses = $this->model_app->edit('taggambar', array('id_taggambar' => $id, 'username' => $this->session->username))->row_array();
            }

            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_taggambar/view_tag_edit', $data);
        }
    }

    function delete_taggambar()
    {
        cek_session_akses('taggambar', $this->session->id_session);
        if ($this->session->level == 'admin') {
            $id = array('id_taggambar' => $this->uri->segment(3));
        } else {
            $id = array('id_taggambar' => $this->uri->segment(3), 'username' => $this->session->username);
        }
        $this->model_app->delete('taggambar', $id);
        redirect('administrator/taggambar');
    }


    // Controller Modul Komentar Gambar

    function komentargambar()
    {
        cek_session_akses('komentargambar', $this->session->id_session);
        $data['record'] = $this->model_app->view_join_one('komentargambar', 'gambar', 'id_gambar', 'id_komentargambar', 'DESC');
        $this->template->load('administrator/template', 'administrator/mod_komentargambar/view_komentargambar', $data);
    }

    function edit_komentargambar()
    {
        cek_session_akses('komentargambar', $this->session->id_session);
        $id = $this->uri->segment(3);
        if (isset($_POST['submit'])) {
            $data = array(
                'nama_komentargambar' => $this->input->post('a'),
                'url' => $this->input->post('b'),
                'isi_komentargambar' => $this->input->post('c'),
                'aktif' => $this->input->post('d')
            );
            $where = array('id_komentargambar' => $this->input->post('id'));
            $this->model_app->update('komentargambar', $data, $where);
            redirect('administrator/komentargambar');
        } else {
            $proses = $this->model_app->edit('komentargambar', array('id_komentargambar' => $id))->row_array();
            $data = array('rows' => $proses);
            $this->template->load('administrator/template', 'administrator/mod_komentargambar/view_komentargambar_edit', $data);
        }
    }

    function delete_komentargambar()
    {
        cek_session_akses('komentargambar', $this->session->id_session);
        $id = array('id_komentargambar' => $this->uri->segment(3));
        $this->model_app->delete('komentargambar', $id);
        redirect('administrator/komentargambar');
    }
